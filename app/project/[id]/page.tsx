...
function hasCompleteMasterPromptPackage(masterPromptPackage: MasterPromptPackage | null) {
  return Boolean(masterPromptPackage?.graders?.length);
}
...
export default function ProjectPage() {
  ...
  const [isCachedMasterPrompt, setIsCachedMasterPrompt] = useState(false);
  ...
  const fallbackGraderNotice = masterPromptPackage?.graderGenerationSource === "deterministic_fallback"
    ? `Graders were generated from your board eval details because the AI grader-writing step was temporarily busy. They are saved and usable.${isAdminBypass ? " Use Force regenerate later if you want AI-authored grader wording." : ""}`
    : null;
  ...
  useEffect(() => {
    if (!isGeneratingMasterPrompt || !masterPromptStartedAt) return;
    const timer = window.setInterval(() => {
      setMasterPromptElapsedSeconds(Math.max(0, Math.floor((Date.now() - masterPromptStartedAt) / 1000)));
    }, 1000);
    return () => window.clearInterval(timer);
  }, [isGeneratingMasterPrompt, masterPromptStartedAt]);
  ...
  async function handleGenerateMasterPromptPackage(forceRegenerate = false) {
    ...
    if (hasCompleteMasterPrompt && !forceRegenerate) {
    ...
    try {
      ...
      if (!response.ok || !payload.masterPromptPackage) {
        if (payload.cachedPackage) {
          ...
          setViewMode("prompt_graders");
          setMasterPromptNotice("Loaded the saved master prompt package. Refresh is no longer required.");
          return;
        }
          ...
          <div className="flex h-full min-h-0 flex-col gap-2">
            ...
            {viewMode === "prompt_graders" ? (
              <section className="min-h-0 flex-1 overflow-y-auto rounded-2xl border border-slate-200/80 bg-white p-6">
                {(!masterPromptPackage || hasPartialMasterPrompt) && !isGeneratingMasterPrompt ? (
                  <div className="mx-auto flex max-w-3xl flex-col gap-4 rounded-2xl border border-dashed border-slate-300 bg-slate-50/70 p-8">
                    <h3 className="text-2xl font-semibold text-slate-900">{hasPartialMasterPrompt ? "Finish generating graders" : "Generate master prompt and graders"}</h3>
                    <p className="text-base leading-7 text-slate-700">{hasPartialMasterPrompt ? "Your master prompt was saved, but grader generation did not complete. Resume generation to fill in the missing graders." : "Turn this ADES board into a build-ready system prompt and OpenAI-compatible graders."}</p>
                    <div className="flex flex-wrap items-center gap-3">
                      <button type="button" onClick={() => void handleGenerateMasterPromptPackage()} disabled={project.status === "generating"} className="ades-primary-btn px-4 py-2.5 text-sm disabled:opacity-60">{hasPartialMasterPrompt ? "Generate missing graders" : "Generate master prompt and graders"}</button>
                      <p className="max-w-2xl text-sm leading-6 text-slate-600">{hasPartialMasterPrompt ? "ADES will reuse the saved master prompt and only complete the grader stage." : "One-time generation per project. ADES uses your saved board, evals, safeguards, and assumptions."}{isAdminBypass ? " Admins can force regenerate." : ""}</p>
                    </div>
                  </div>
                ) : null}
