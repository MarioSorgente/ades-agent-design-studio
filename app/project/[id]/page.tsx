...
export default function ProjectPage() {
  ...
  async function handleGenerateBoard() {
    ...
    try {
      ...
      setProject((previous) =>
        previous
          ? { ...previous, title: payload.project.title, summary: payload.project.summary, status: "generated", ideaPrompt, audience, constraints, critique: null }
          : previous,
      );
    } catch (error) {
  ...
  async function handleImportJson(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setProject((previous) =>
        previous
          ? { ...previous, title: parsed.title || previous.title, summary: parsed.summary, ideaPrompt: parsed.ideaPrompt, audience: parsed.audience, constraints: parsed.constraints, critique: parsed.critique }
          : previous,
      );
  ...
  return (
    <AppShell
      ...
      subtitle="Board-first studio with progressive disclosure for quality loops, evals, and business context."
        ...
        {!isLoading && project ? (
          <div className="space-y-3">
            <section className="rounded-2xl border border-slate-200/80 bg-white/90 p-3 shadow-[0_8px_24px_-24px_rgba(15,23,42,0.9)]">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="rounded-full border border-indigo-200 bg-indigo-50 px-3 py-1 text-[11px] font-semibold uppercase tracking-wide text-indigo-700">Main flow first</span>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  {BOARD_VIEW_MODES.map((mode) => (
                    <button
                      ...
                      className={viewMode === mode ? "ades-primary-btn px-3 py-1.5 text-[11px]" : "ades-ghost-btn px-3 py-1.5 text-[11px]"}
                    >
                      {mode === "main" ? "Main flow" : mode[0].toUpperCase() + mode.slice(1)}
                    </button>
                  ))}
                  <button type="button" className="ades-ghost-btn px-3 py-1.5 text-[11px]" onClick={() => setIsBottomPanelOpen((prev) => !prev)}>
                    {isBottomPanelOpen ? "Hide details" : "Details"}
                  </button>
                </div>
              </div>
            </section>
            ...
            <section className="flex gap-3">
              <aside className={`rounded-2xl border border-slate-200/80 bg-white/85 p-2 shadow-[0_8px_30px_-24px_rgba(15,23,42,0.8)] ${isLeftToolbarExpanded ? "w-[204px]" : "w-[52px]"}`}>
                <button
                  type="button"
                  onClick={() => setIsLeftToolbarExpanded((prev) => !prev)}
                  className="mb-2 flex w-full items-center justify-center rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-[10px] font-semibold text-slate-600"
                >
                  {isLeftToolbarExpanded ? "Collapse" : "☰"}
                </button>
                <div className="space-y-1.5">
                  {CORE_NODE_TYPES.map((type) => {
                    ...
                    return (
                      <button
                        ...
                        className="flex w-full items-center gap-2 rounded-xl border border-slate-200 bg-white px-2 py-2 text-left text-xs font-medium text-slate-700 transition hover:border-indigo-200 hover:bg-indigo-50/30"
                        title={theme.label}
                      >
                  ...
                  <button type="button" onClick={deleteSelectedNode} className="flex w-full items-center justify-center rounded-xl border border-rose-200 bg-rose-50 px-2 py-2 text-xs font-semibold text-rose-700">
              ...
              <div className="relative min-w-0 flex-1">
                ...
                {!isRightPanelPinned && selectedNodeId ? (
                  <aside className="absolute right-3 top-3 z-10 w-[320px] rounded-2xl border border-slate-200/90 bg-white/95 p-3 shadow-[0_20px_45px_-34px_rgba(15,23,42,0.7)] backdrop-blur">
                    <div className="mb-3 flex items-center justify-between">
                      <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Inspector</p>
                      <button type="button" onClick={() => setIsRightPanelPinned(true)} className="ades-ghost-btn px-2 py-1 text-[10px]">
                        Pin
                      </button>
                    </div>
                    <BoardInspector />
              ...
              {isRightPanelPinned ? (
                <aside className="w-[336px] rounded-2xl border border-slate-200/80 bg-white/92 p-3 shadow-[0_10px_32px_-28px_rgba(15,23,42,0.8)]">
                  <div className="mb-3 flex items-center justify-between">
                    <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Inspector</p>
                    <button type="button" onClick={() => setIsRightPanelPinned(false)} className="ades-ghost-btn px-2 py-1 text-[10px]">
                      Unpin
                    </button>
                  </div>
                  <BoardInspector />
                ...
                <button
                  ...
                  className="h-fit rounded-2xl border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-600 shadow-sm"
              ...
              <section className="rounded-2xl border border-slate-200/80 bg-white/90 p-3">
                <div className="grid gap-3 md:grid-cols-3">
                  <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                    ...
                    <label className="mt-2 block text-[11px] font-semibold uppercase tracking-wide text-slate-500">
                      Constraints
                      <textarea className="ades-input mt-1 min-h-[66px] resize-y" value={constraints} onChange={(event) => setConstraints(event.target.value)} placeholder="Policy, compliance, latency, budget..." />
                    </label>
                    <button type="button" onClick={() => void handleGenerateBoard()} disabled={isGenerating || !ideaPrompt.trim()} className="ades-primary-btn mt-2 w-full px-3 py-2 text-xs disabled:cursor-not-allowed disabled:opacity-60">
                  ...
                  <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <button type="button" onClick={() => void handleCritiqueBoard()} disabled={isCritiquing || nodes.length === 0} className="ades-primary-btn px-3 py-1.5 text-[11px] disabled:cursor-not-allowed disabled:opacity-60">
