...
export function createNodeData(type: AdesNodeType, label: string): AdesNodeData {
  ...
  return {
    ...
    evalName: isEval ? "Define the output check" : "",
    evalQuestion: isEval ? "Name the expected output and the observable result a grader should see." : "",
    evalMetric: isEval ? "List concrete failure examples for this output." : "",
    evalCategory: "task_success",
    evalScope: "step",
    evalCriteria: isEval ? "State the specific fact, structure, or decision required for a pass." : "",
    evalDataset: isEval ? "Define the evaluation set, including normal, failure, and boundary cases." : "",
    evalMethod: "",
    evalThreshold: isEval ? "State how many failures are acceptable in the defined evaluation set." : "",
