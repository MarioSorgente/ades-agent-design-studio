...
export function StudioBoard({ className, viewMode = "flow", selectedNodeId, isDetailsPanelOpen = false, detailsInsetPx = 0, onSelectNode, onAddStepAt, onAddStepToEnd, onDuplicateStep, onDeleteNode, onAddConnectedNode, onOpenDetails, onAddNotice }: StudioBoardProps) {
    ...
    () =>
      orderedMainSteps.map((step) => {
        ...
        return {
          ...
          feedbackNodes: connected.filter((node) => node.type === "feedback"),
  ...
  function fitFlow() {
    ...
    const boardWidth = Math.max(420, maxX - minX + 520);
  ...
  function handleAddConnected(stepId: string, kind: AttachmentKind) {
    const map: Record<AttachmentKind, { type: AdesNodeType; message: string }> = {
      ...
      feedback: { type: "feedback", message: "Feedback loop added" },
    ...
    >
            ...
            <div className="mx-auto min-w-max">
              <div className="flex items-start gap-5">
                ...
                {flowRows.map((row, index) => {
                  const forcedAttachment = selectedAttachmentParentStepId === row.step.id ? (nodeById.get(selectedNodeId || "")?.type === "eval" ? "evals" : nodeById.get(selectedNodeId || "")?.type === "reflection" ? "reflections" : nodeById.get(selectedNodeId || "")?.type === "feedback" ? "feedback" : nodeById.get(selectedNodeId || "")?.type === "risk" ? "risks" : null) : null;
                  const openAttachment = openAttachments[row.step.id] ?? forcedAttachment;
                  const isSelected = selectedNodeId === row.step.id;
                  return (
                    <div key={row.step.id} className="flex items-start gap-4">
                      <div ref={(el) => { stepRefMap.current[row.step.id] = el; }} className="relative pt-10 pb-36">
                        <button type="button" onClick={() => onSelectNode(row.step.id)} onDoubleClick={() => onOpenDetails(row.step.id)} className={`relative z-20 w-[350px] rounded-2xl border bg-white p-4 text-left shadow-[0_16px_36px_-30px_rgba(15,23,42,0.65)] ${isSelected ? "border-indigo-300 ring-2 ring-indigo-100" : "border-slate-200 hover:border-indigo-200"}`}>
                          ...
                          <div className="absolute -top-1 right-2 z-50 flex items-center gap-1 rounded-lg border border-indigo-200 bg-white p-1 shadow-lg">
                            ...
                            <details className="relative" onClick={(event) => event.stopPropagation()}>
                              ...
                              <div className="absolute right-0 top-full z-30 mt-1 w-44 rounded-lg border border-slate-200 bg-white p-1.5 shadow-lg">
                                ...
                                <button type="button" className="ades-ghost-btn mt-1 w-full px-2 py-1 text-left text-xs" onClick={() => handleAddConnected(row.step.id, "feedback")}>Feedback loop</button>
                        ...
                        {openAttachment ? (
                          <div className="absolute left-0 top-full z-40 mt-4 w-[350px] rounded-xl border border-slate-200 bg-white p-3 shadow-lg">
                            <div className="absolute -top-4 left-1/2 h-4 w-px -translate-x-1/2 bg-slate-300" />
                            {openAttachment === "evals" ? <AttachmentList items={row.evalNodes.map((node) => ({ id: node.id, label: node.data.evalQuestion || node.data.evalName || "Eval", subLabel: `Threshold: ${node.data.evalThreshold || "Add threshold"} · Pass: ${node.data.evalCriteria || "Add pass criteria"}` }))} emptyMessage="No eval cards attached yet." onSelectNode={onSelectNode} onOpenDetails={onOpenDetails} /> : null}
                              ...
                              <div className="space-y-2">
                                {row.reflectionNodes.length ? row.reflectionNodes.map((node) => (
                                  <button key={node.id} type="button" onClick={() => { onSelectNode(node.id); onOpenDetails(node.id); }} className="w-full rounded-lg border border-indigo-200 bg-indigo-50/70 p-2.5 text-left hover:border-indigo-300">
                                    <p className="text-xs font-semibold text-indigo-900">{node.data.label || "Self-critique"}</p>
                                    <p className="text-xs text-indigo-800">{node.data.reflectionPrompt || "Critique this step before moving forward."}</p>
                                    <p className="mt-2 text-[11px] font-medium text-indigo-700">{node.data.reflectionLoopTarget === "previous_step" ? "returns to previous step" : "returns to this step"}</p>
                                  </button>
                                )) : <p className="text-xs text-slate-600">No reflections attached yet.</p>}
                              </div>
                            ) : null}
                            {openAttachment === "feedback" ? <AttachmentList items={row.feedbackNodes.map((node) => ({ id: node.id, label: node.data.label || "Feedback", subLabel: node.data.feedbackCondition || "Trigger not defined" }))} emptyMessage="No feedback loops attached yet." onSelectNode={onSelectNode} onOpenDetails={onOpenDetails} /> : null}
                            {openAttachment === "risks" ? <AttachmentList items={row.riskNodes.map((node) => ({ id: node.id, label: node.data.label || "Safeguard", subLabel: node.data.confidenceCheck || "Mitigation not defined" }))} emptyMessage="No safeguards attached yet." onSelectNode={onSelectNode} onOpenDetails={onOpenDetails} /> : null}
                          </div>
                        ) : null}
                      </div>
      ...
      <div className="pointer-events-none absolute bottom-4 left-4 z-30">
        <div className="pointer-events-auto flex items-center gap-1 rounded-xl border border-slate-200 bg-white/95 p-1 shadow-sm">
          ...
          <button type="button" className="ades-ghost-btn h-7 px-2 py-0 text-xs" onClick={fitFlow}>Fit</button>
        </div>
      </div>
    </div>
  );
